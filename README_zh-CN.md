...
## R、R Markdown 和 Notebook 工作流

R 相关环境统一写在 `modules/home/development/toolchain.nix` 中，通过 `rWrapper` 和 `rstudioWrapper` 提供，不建议在 RStudio 里手动 `install.packages()` 来维护主环境。

当前 wrapper 内置了常用课程和绘图包：

- R Markdown / 报告：`rmarkdown`、`knitr`、`tinytex`
- VSCode / Jupyter 支持：`IRkernel`、`languageserver`
- Tidyverse / 绘图：`tidyverse`、`ggplot2`
- 统计计算课程常用包：`boot`、`bootstrap`、`MASS`、`Matrix`、`survival`、`car`、`lmtest`、`sandwich`、`lme4`
- 地图测试：`maps`、`mapdata`

当前没有加入 `httpgd`，因为这个 nixpkgs 版本里的 `r-httpgd` 被标记为 broken，会导致 `nixos-rebuild` 直接失败。R Notebook 的图形内联显示可以先通过 `IRkernel` 来完成。

重建系统后，先注册一次 R kernel：
...
如果想要类似 Jupyter 的实时图形反馈，用 VSCode 打开：

```text
/home/eurekaimer/Documents/Rcode/r_china_charts_notebook.ipynb
```

右上角选择 `R` kernel，然后逐个 cell 运行。

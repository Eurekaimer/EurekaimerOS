...
用户桌面入口是 [`modules/home/desktop.nix`](../modules/home/desktop.nix)，当前只启用 Niri 与 Noctalia，不会重复启动另一套面板或启动器。
...
+ [`config/noctalia-config/settings.json.in`](../modules/home/config/noctalia-config/settings.json.in)
  ...
  + 壁纸渲染已禁用，减少重复桌面合成；通知声音关闭；壁纸由 Niri 会话中的 swww 设置。
...
+ [`core/ui.nix`](../modules/home/core/ui.nix)
  + GTK 2/3/4 默认字体为 LXGW WenKai 11。

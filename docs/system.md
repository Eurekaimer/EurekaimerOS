...
+ [`locale.nix`](../modules/system/locale.nix)
  ...
  + Keeps JetBrains Mono/Sarasa Mono for terminals and source code to preserve alignment.
...
+ [`power.nix`](../modules/system/power.nix)
  ...
  + Disables Bluetooth at boot and when switching to battery; users may re-enable it when required.
...
+ [`virtualisation.nix`](../modules/system/virtualisation.nix) enables [libvirt](https://libvirt.org/), [virt-manager](https://virt-manager.org/), QEMU, swtpm, SPICE USB redirection, and Windows virtio drivers; unused `virtchd` is disabled.
